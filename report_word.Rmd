...
if(FALSE){
    ...
    newdiag_before = "No"
    report_sections = c("Exposure", "Response", "Health", "Survival Analysis", "Poisson", "Data Avaibility")
...
```{r, echo=FALSE, eval = ("Exposure" %in% params$report_sections)}
asis_output("Exposure diagnoses includes following diagnoses. In this table all cases are listed (for example if same person has ICD-8, ICD-9 and ICD-10 diagnose, all those cases will be listed). **Full list is included in Appendix section.**\\n")
```

```{r selected_diagnoses_chunk, echo=FALSE, eval = ("Exposure" %in% params$report_sections)}
## TODO Option to list only 10 diagnoses and full list in appendix
selected_diagnoses1 <-  table_summary_diagnoses(dpop, exposure_diagnoses, group = "exposure")
knitr::kable(head(selected_diagnoses1, 10))
```
...
```{r, echo=FALSE, eval = ("Response" %in% params$report_sections)}
asis_output("Response diagnoses includes following diagnoses on exposure group. Here is listed only 10 diagnoses and **Full list is included in Appendix section.**")
```
...
knitr::kable(head(selected_diagnoses2, 10))
```
...
```{r crosstab, echo = FALSE, eval = ("Response" %in% params$report_sections), results = 'asis'}
...
```{r, echo=FALSE, eval = ("Exposure" %in% params$report_sections)}
asis_output("### Exposure Diagnoses\\n")
```

```{r, echo=FALSE, eval = ("Exposure" %in% params$report_sections)}
## TODO Option to list only 10 diagnoses and full list in appendix
knitr::kable(selected_diagnoses1)
```


```{r, echo=FALSE, eval = ("Response" %in% params$report_sections)}
asis_output("### Response Diagnoses\\n")
```
